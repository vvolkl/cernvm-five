...
dnf install -y --installroot=$BUILD_DIR --releasever=/ --nodocs https://research.cs.wisc.edu/htcondor/repo/current/htcondor-release-current.el9.noarch.rpm 
dnf install -y --installroot=$BUILD_DIR --releasever=/ --nodocs condor 
dnf install -y --installroot=$BUILD_DIR --releasever=/ --nodocs cloud-init passwd
